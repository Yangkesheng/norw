...
生产者向 broker 发送消息时，broker 可以返回一个成功响应码或者一个错误响应码。错误响应码可以分为两种，一种是在重试之后可以解决的，还有一种是无法通过重试解决的。

### 消费者的可靠性配置

1. Group.id: 如果希望消费者可以看到主题的所有消息，需要设置唯一的group.id。如果两个消费者具有相同的group.id，并且订阅了同一个主题，那么每个消费者会分到主题分区的一个子集
2. atuo.offset.reset: 这个参数指定了没有偏移量可提交时（比如消费者第一次启动时）或者请求的偏移量在borker不存在时。 earliest,消费者读取大量的重复数据。latest,如果选择了这种配置，消费者会从分区的末尾开始读取数据，这样可以减少重复处理消息，但很可能会错过一些消息。
3. enable.auto.commit：让消费者基于任务调度自动提交偏移量，也可以在代码里手动提交偏移量。
4. auti.commit.interval.ms与第3个参数有直接的联系。设置自动提交偏移量的频度。默认5秒
